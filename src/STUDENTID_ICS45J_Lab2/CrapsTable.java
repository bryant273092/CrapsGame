    ...
    implements ActionListener
{
    ...
    {
    	if(Integer.parseInt(display.getCredit().getText())>0) {
	        // You write code here
	        // The following should only run if there is a valid bet
	    		if (Integer.parseInt(display.getBet().getText())<=Integer.parseInt(display.getCredit().getText()) && Integer.parseInt(display.getBet().getText())>0 ) {
	        ...
	        // If there is NOT a valid bet, a message should give a range of a valid bet
	    			...
	    			JOptionPane.showMessageDialog(null,"Error: bet must greater than 0 and less than or equal to "+display.getCredit().getText());
	    		}
    	}
    	else {
